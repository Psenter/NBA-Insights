...
function PlayerDetail() {
  ...
  useEffect(() => {
    if (id) {
      ...
      axios
        .get(`http://127.0.0.1:8000/Players/${id}/?format=json`)
        .then((response) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          const teamForPlayer = response.data.find((team) =>
            team.players.includes(parseInt(id, 10))
          );
  ...
  const handleAddFavoritesP = async () => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/FavoritePlayers/",
        favoriteData
      );
  ...
  return (
    <div>
      <div className="container">
        <div className="row">
          <div className="col-11">
            {state.user ? (
              <button
                onClick={handleAddFavoritesP}
                disabled={addingToFavoritesP}
      ...
      <div className="row text-center mt-5 mb-5 display-1 pb-4">
        <div className="col-2"></div>
        <div className="col-8 display-1 border-bottom border-dark">
      ...
      <div className="row text-center mt-5 mb-5 display-6">
        ...
        <Link
          className="col-4 link-gone"
          href={`/[id]`}
          as={`/${playerTeam.id}`}
        >
          Team: {playerTeam.team_name}
        </Link>
      </div>
      <div className="row text-center mt-5 mb-5 display-6">
