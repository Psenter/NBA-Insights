...
function TeamDetail() {
  ...
  useEffect(() => {
    if (id) {
      axios
        .get(`http://127.0.0.1:8000/Teams/${id}/?format=json`)
        .then((response) => {
  ...
  const handleAddFavorites = async () => {
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/FavoriteTeams/",
        favoriteData
      );
    ...
    <div>
      ...
      <div className="container">
        ...
        <div className="col-12 text-center mt-5">
          ...
          <div className="display-1 border-bottom border-dark">
        ...
        <div>
          {teamPlayers.map((player) => (
            <div className="row text-center display-6 mb-4" key={player.id}>
              <Link className="col-4 link-gone" href={`/player/${player.id}`}>
...
function TeamMedia({ mediaId }) {
  ...
  useEffect(() => {
    if (mediaId) {
      axios
        .get(`http://127.0.0.1:8000/Media/${mediaId}/?format=json`)
        .then((response) => {
  ...
  return (
    <div>
      {assetUrl && (
        <div>
          <img className="img-bigger" src={assetUrl} alt="Team Asset" />
        </div>
      )}
    </div>
  );
